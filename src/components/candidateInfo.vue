<template>
  <v-card class="ma-1">
      ...
      <v-layout text-xs-center row wrap>
        ...
        <v-flex xs12>
          <a :href="'http://alerce.online/object/'+alerce">
            <span class="caption">{{alerce}}</span>
          </a>
          <v-divider></v-divider>
        </v-flex>
      </v-layout>
    ...
    <v-card-text>
      <v-layout text-xs-center row wrap>
        <v-flex xs7>
          <span class="font-weight-black">Disc. Date:</span>
          {{data["Discovery Date (UT)"]}} (UT)
        </v-flex>
        <v-flex xs5>
          <span class="font-weight-black">Mag:</span>
          {{data["Discovery Mag"]}} (
          <span class="font-weight-black">Band:</span>
          {{data["Discovery Mag Filter"].replace("-ZTF","")}})
        </v-flex>
        <v-flex xs7>
        ...
        <v-flex xs12>
          ...
          <div v-else>
            ...
            <v-tooltip v-if="last_non_detection" bottom>
              <template v-slot:activator="{ on }">
                <span v-on="on">
                  ...
                  >Rise ({{first_detection.fid =="1"? "g-ZTF": "r-ZTF"}}):</span>
            ...
            <div v-else>No non-detections before first detection</div>
          </div>
        </v-flex>
        <v-flex v-if="!loading" xs12>
          <h5>Discovery Stamps</h5>
        </v-flex>
        <v-flex xs4>
          <v-img :src="scienceStamp"></v-img>Science
        </v-flex>
        <v-flex xs4>
          <v-img :src="templateStamp"></v-img>Template
        </v-flex>
        <v-flex xs4>
          <v-img :src="differenceStamp"></v-img>Difference
        </v-flex>
      </v-layout>
    </v-card-text>
  </v-card>
...
export default {
  ...
  computed: {
    ...
    alerts() {
      return {
        ...
        non_detections: this.non_detections,
      };
    },
    differenceStamp() {
      ...
      return (
        base_url +
        "?oid=" +
        this.first_detection.oid +
        "&candid=" +
        this.first_detection.candid_str +
  ...
  mounted: function () {
    ...
    axios
      .post(ztf_url + "/get_detections", { oid: this.alerce })
      .then(function (response) {
        self.detections = response.data.result.detections;
      });
    axios
      .post(ztf_url + "/get_non_detections", { oid: this.alerce })
      .then(function (response) {
    ...
    alerts: function (newVal) {
      if (newVal.detections.length > 0 && newVal.non_detections.length > 0) {
        ...
        newVal.non_detections.forEach(function (non_detection) {
          if (
            non_detection.fid == first_detection.fid &&
            non_detection.mjd < first_detection.mjd &&
            last_non_detection.mjd < non_detection.mjd
          ) {
